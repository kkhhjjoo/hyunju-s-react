...
export function getAxiosInstance() {
  ...
  instance.interceptors.response.use((response) => {
    ...
    if (response?.state === 401) { //인증 실패
      if (config.url === REFRESH_URL) { //refresh token도 만료된 경우 로그인 페이지로
        navigateLogin();
      } else if (user) { //로그인 했으나 access token이 만료된 경우 accesss token과 refresh token 재발급
        ...
        //갱신되 accessToken으로 실패했던 요청을 다시 시도
  ...
  function navigateLogin() { 
    ...
    if (gotoLogin) { 
      ...
      router.navigate('/user/login', { state: {from: router.state.location.pathname} })
    }
  }
