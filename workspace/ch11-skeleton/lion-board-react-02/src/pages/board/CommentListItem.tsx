    ...
    <div className="shadow-md rounded-lg p-4 mb-4">
      <div className="flex justify-between items-center mb-2">
        <div className="flex items-center">
          <img className="w-8 mr-2 rounded-full" src={reply.user.image} alt={`${reply.user.name} 프로필 이미지`} />
          <Link to="" className="text-orange-400">
            { reply.user.name}
          </Link>
        </div>
        <time className="text-gray-500" dateTime={ reply.updatedAt }>
          { reply.updatedAt}
        </time>
      </div>
      <div className="flex justify-between items-start mb-2">
        <p className="whitespace-pre-wrap text-sm flex-1">{ reply.content }</p>
        <form action="#" className="inline ml-2" onSubmit={onSubmit}>
