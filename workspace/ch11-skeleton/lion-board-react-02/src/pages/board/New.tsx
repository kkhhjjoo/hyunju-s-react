...
const New = () => {
  ...
  const { mutate } = useMutation<PostInfoRes, AxiosError<ErrorRes>, FormData>({
    ...
    onSuccess: (newPost) => { 
      alert(`${newPost.item._id}번 글이 등록되었습니다.`);
      ...
      <section className="mb-8 p-4">
        <form action="/info/1" onSubmit={handleSubmit(onSubmit)}>
          <div className="my-4">
            ...
            <input id="title" type="text" placeholder="제목을 입력하세요." className="w-full py-2 px-4 border rounded-md dark:bg-gray-700 border-gray-300 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200" {...register('title', {
              required: '제목을 입력하세요.'
            })} />
          ...
          <div className="flex justify-end my-6">
            ...
            <a href="/info" className="bg-gray-900 py-1 px-4 text-base text-white font-semibold ml-2 hover:bg-amber-400 rounded">
              취소
            </a>
          </div>
        </form>
