...
//react-router의 loader에서 사용할 등록 함수
export async function todoCreateAction({request}:ActionFunctionArgs) {
  try {
    const formData = await request.formData();
    await createTodo(formData);
    return redirect(`/todolist/todolist`); //loader, action에서 페이지 이동에 사용
  } catch (err) {
...
//react-router의 loader에서 사용할 삭제 함수
