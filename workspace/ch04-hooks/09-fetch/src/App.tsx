...
const App = () => {
  ...
  서버에서 정상 응답을 방을 경우 "로딩중..." 메시지를 해제하고 할 일 목록을 출력
  서버에서 에러를 응답받을 경우 "로딩중..." 메시지를 해제하고 에러메시지를 출력
  */
  ...
  const fetchTodo = async (url: string) => {
    try {
      ...
      //성공, 실패 여부와 관계없이 로딩상태를 false로 설정
      setLoading(false);
      //로딩메시지를 없애라
    }
  ...
  return (
    <>
      ...
      <ul>
        {data && data?.items.map(item => <li key={ item._id }>{ item.title }</li>)}
      </ul>
    </>
  )
}
